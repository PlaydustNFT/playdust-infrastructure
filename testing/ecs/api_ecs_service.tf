resource "aws_ecs_service" "api_ecs_service" {
  ...
  lifecycle {
    ignore_changes = [task_definition,health_check_grace_period_seconds,propagate_tags,tags,capacity_provider_strategy,deployment_circuit_breaker,deployment_controller,network_configuration ]
  }
}
